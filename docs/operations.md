...
本来の検知手段は画面に常時表示される**最終収集成功時刻**（ADR-0007、#20）だが、
**#20 は本書作成時点（2026-09-20）でまだ未実装**である。#20 が入ったら、
画面を開くたびにこの時刻が「今から 1〜2 時間以内」であることを確認する運用に切り替える
（`COLLECT_CRON` の既定は毎時なので、正常なら常に 1 時間強以内のはず）。

#20 が入るまでの代替手段として、`collection_cursors` テーブルを直接見る。
...
`last_success_at` が更新されていない、または `last_error` に
`認証に失敗した。PAT が無効か期限切れの可能性がある`（401）が出ていたら、
PAT の期限切れを疑う。403 の場合は期限切れではなく、対象リポジトリの選択漏れや
権限設定ミスの可能性が高い（エラーメッセージにヒントが出る。`src/github/client.ts` の
`describeStatus` を参照)。
...
4. 最終収集成功時刻が更新されることを確認する（#20 が入っていれば画面で、
   入っていなければ「1.3」の DB クエリで）。起動時に自動で 1 回収集が走るため
   （`COLLECT_ON_STARTUP`）、数秒〜数十秒待って確認する。
...
- 画面（または #20 実装前は DB クエリ）で件数・最終収集成功時刻を確認すること
